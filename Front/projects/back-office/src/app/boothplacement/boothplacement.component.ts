...
for (let i = 0; i < this.initialPositionsBigBooths.length; i++) {
  ...
  this.booth.boothname = this.BigboothNames[i]; // Assuming boothNames is an array
  this.booth.x = index.x;
  this.booth.y = index.y;
  this.booth.boothType = 'big'
  const boothbig = this.findBoothByName(this.booth);
      console.log("found booth !")
      console.log(boothbig)
      
      if (boothbig) {
        this.booth.id= boothbig.id
      
      } 
  console.log(this.booth)
  this.serviceBooth.AddBooth(this.booth).subscribe((response:any) =>{
    console.log(response)
  });
}
    ...
    if (result.isConfirmed) {
      if(boothindex !== -1){
        this.serviceBooth.deleteBooth(booth).subscribe((response : any) =>{
          ...
          this.boothNames.splice(boothindex, 1);
